...
- 🎯 **批量LLM解析** — 支持选择章节范围, 批量队列转化, 全程显示日志和进度, 支持并发控制和取消
- ⏭️ **跳过已解析章节** — 批量 LLM 解析时自动跳过已有台词的章节, 防止重复生成(默认开启, 可关闭)
- 🎙️ **批量TTS配音** — 按章节一键配音, WebSocket 实时推送进度
- ⚡ **异步协程引擎** — LLM/TTS 引擎全面改为 async 非阻塞调用, 数据库连接池异步化, 高并发下不再阻塞线程
- 🔄 **LLM + TTS 并行流水线** — 一键挂机模式下 LLM 解析与 TTS 配音并行执行, 大幅缩短整体耗时
- 🎭 **路人语音池** — 为项目配置路人语音池, 自动随机分配音色给未绑定角色, 角色按对话次数排序
...
- � **合并导出分段** — 合并导出支持按时长分段(以章节为最小单位, 不截断对话), 支持范围选择
...
- 🔌 **Index-TTS 一键安装** — 提供安装脚本, 支持 8GB 显存模式
- 🎤 **音色上传管理** — 支持上传自定义参考音频创建/编辑音色, 音色卡片支持试听和删除
- 🧠 **LLM 解析重试与限流退避** — 批量解析遇到速率限制自动指数退避重试, 每段最多重试 3 次
- 🎭 **复合情绪支持** — 情绪列表扩展复合情绪(疑惑/紧张/无奈/得意/嘲讽等), 提示词强制 LLM 严格从情绪列表选择
- 🔊 **TTS 多情绪向量** — Index-TTS 支持多维度情绪向量传递, 包括基础和复合情绪映射
- 🔄 **重新解析自动清旧** — 批量 LLM 重新解析章节时自动清除旧台词, 避免重复叠加
- 🎨 **UI 批量操作优化** — 批量 LLM/TTS/一键挂机弹窗支持 Drawer 抽屉模式, 章节范围选择更灵活

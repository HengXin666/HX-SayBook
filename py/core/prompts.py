...
def get_context2lines_prompt(
    possible_characters, novel_content, possible_emotions, possible_strengths
) -> str:
...
情绪以及情绪强弱识别:
根据上下文场景，识别出每条台词所对应的情绪以及情绪强度。
**极其重要**：emotion_name 和 strength_name 的值必须严格且完全地来自下方提供的情绪列表possible_emotions和情绪强度列表possible_strengths中的某一项，禁止使用列表之外的任何词语，禁止自由发挥或创造新情绪词。
当台词的语气更贴合列表中某个复合情绪时，应优先选择复合情绪，以获得更精确的情感表达。
...
{"role_name": "旁白", "text_content": "此时，张三愤怒站着，李四不知所措地后退了一步", "emotion_name": "平静", "strength_name": "中等"},
{"role_name": "李四", "text_content": "这可不管我的事儿", "emotion_name": "紧张", "strength_name": "中等"},
{"role_name": "王五", "text_content": "行了行了，你们俩能不能消停点", "emotion_name": "无奈", "strength_name": "较强"}
]
</result>
...
def get_prompt_str():
    ...
    2. **极其重要**：emotion_name 和 strength_name 的值必须严格且完全地来自下方提供的情绪列表（possible_emotions）与强度列表（possible_strengths）中的某一项，禁止使用列表之外的任何词语，禁止自由发挥或创造新情绪词。
    3. 当台词的语气更贴合列表中某个复合情绪时，应优先选择复合情绪，以获得更精确的情感表达。
    4. "旁白"内容的情绪与强度统一为：情绪"平静"，强度"中等"。
    5. 情绪识别不得影响或改写原文内容，仅用于标注。
    特殊情况处理：
    ...
    [
      ...
      {"role_name": "灰衣少年", "text_content": "牧哥可是被选拔出来参加过"灵路"的人，我们整个北灵境中，可就牧哥一人有名额，你们应该也知道参加"灵路"的都是些什么变态吧？当年我们这北灵境可是因为此事沸腾了好一阵的，从那里出来的人，最后基本全部都是被"五大院"给预定了的。", "emotion_name": "得意", "strength_name": "中等"}
    ]

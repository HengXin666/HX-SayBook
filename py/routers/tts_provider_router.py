...
from py.dto.tts_provider_dto import TTSProviderCreateDTO, TTSProviderResponseDTO
...
@router.get(
    "/{tts_provider_id}",
    response_model=Res[TTSProviderResponseDTO],
    summary="查询TTS供应商",
    description="根据TTS供应商ID查询TTS供应商信息",
)
def get_tts_provider(
    tts_provider_id: int, service: TTSProviderService = Depends(get_service)
):
...
@router.get(
    "",
    response_model=Res[List[TTSProviderResponseDTO]],
    summary="查询所有TTS供应商",
...
@router.put(
    "/{tts_provider_id}",
    response_model=Res[TTSProviderCreateDTO],
    summary="修改TTS供应商",
    description="根据TTS供应商ID修改TTS供应商信息",
)
def update_tts_provider(
    tts_provider_id: int,
    dto: TTSProviderCreateDTO,
...
def test_tts_provider(
    dto: TTSProviderCreateDTO, service: TTSProviderService = Depends(get_service)
):
    """
    测试tts是否正常
    """
    entity = TTSProviderEntity(**dto.dict())
    success, msg = service.test_tts_provider(entity)
    if success:
        return Res(data=None, code=200, message=msg)
    else:

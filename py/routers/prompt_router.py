...
from py.dto.prompt_dto import PromptCreateDTO, PromptResponseDTO
from py.entity.prompt_entity import PromptEntity
...
@router.post(
    "",
    response_model=Res[PromptResponseDTO],
    summary="创建提示词",
    ...
    try:
        ...
        if entityRes is not None:
            ...
            return Res(
                data=None,
                code=400,
                message=f"创建失败,可能是不存在该任务或提示词数据不完整",
            )
...
@router.get(
    "",
    response_model=Res[List[PromptResponseDTO]],
    summary="查询所有提示词",
...
@router.put(
    "/{prompt_id}",
    response_model=Res[PromptCreateDTO],
    summary="修改提示词",
    description="根据提示词ID修改提示词信息",
)
def update_prompt(
    prompt_id: int, dto: PromptCreateDTO, service: PromptService = Depends(get_service)
):
    ...
    if success:
        ...
        return Res(
            data=None, code=400, message="更新失败,可能是不存在该任务或提示词数据不完整"
        )

...
from py.db.database import Base
...
class LinePO(Base):
    ...
    __table_args__ = (Index("idx_chapter_order", "chapter_id", "line_order"),)
...
class LLMProviderPO(Base):
    ...
    custom_params = Column(
        Text,
        nullable=False,
        default='{"response_format": {"type": "json_object"}, "temperature": 0.7, "top_p": 0.9}',
    )
    # 时间戳
    created_at = Column(
        DateTime, default=lambda: datetime.now(timezone.utc), nullable=False
